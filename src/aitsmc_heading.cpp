...
class AdaptiveSlidingModeControl {
 public:
  ...
  AdaptiveSlidingModeControl() {
    ...
    right_thruster_pub =
        n.advertise<std_msgs::Float32>("/motors/right_thrust", 10);
    left_thruster_pub =
        n.advertise<std_msgs::Float32>("/motors/left_thrust", 10);

    surge_gain_pub =
        n.advertise<std_msgs::Float64>("/usv_control/aitsmc/speed_gain", 10);
    surge_error_pub = n.advertise<std_msgs::Float64>(
        "/usv_control/controller/speed_error", 10);
    surge_sigma_pub =
        n.advertise<std_msgs::Float64>("/usv_control/aitsmc/speed_sigma", 10);
    yaw_sigma_pub =
        n.advertise<std_msgs::Float64>("/usv_control/aitsmc/heading_sigma", 10);
    yaw_gain_pub =
        n.advertise<std_msgs::Float64>("/usv_control/aitsmc/heading_gain", 10);
    yaw_error_pub = n.advertise<std_msgs::Float64>(
        "/usv_control/controller/heading_error", 10);
    control_input_pub = n.advertise<geometry_msgs::Pose2D>(
        "/usv_control/controller/control_input", 10);
    edot_psi_pub = n.advertise<std_msgs::Float64>(
        "/usv_control/aitsmc/debug/edot_psi", 10);

    // ROS Subscribers
    desired_speeds_sub =
        n.subscribe("/desired_values", 10,
                    &AdaptiveSlidingModeControl::desiredTrajCallback, this);
    desired_speedsdot_sub =
        n.subscribe("/desired_values_derivative", 10,
                    &AdaptiveSlidingModeControl::desiredTrajDotCallback, this);
    // ins_pose_sub = n.subscribe("/vectornav/ins_2d/NED_pose", 10,
    // &AdaptiveSlidingModeControl::insCallback, this); local_vel_sub =
    // n.subscribe("/vectornav/ins_2d/local_vel", 10,
    // &AdaptiveSlidingModeControl::velocityCallback, this);
    odom_sub = n.subscribe("/imu/converted_odometry", 10,
                           &AdaptiveSlidingModeControl::odomCallback, this);
    // flag_sub = n.subscribe("/arduino_br/ardumotors/flag", 10,
    // &AdaptiveSlidingModeControl::flagCallback, this); ardu_sub =
    // n.subscribe("arduino", 10, &AdaptiveSlidingModeControl::arduinoCallback,
    // this);
  ...
  void odomCallback(const nav_msgs::Odometry::ConstPtr& o) {
    tf::Quaternion q(o->pose.pose.orientation.x, o->pose.pose.orientation.y,
  ...
  void control() {
    if (testing == 1) {
      ...
      f_u = (((m - Y_v_dot) * v * r + (Xuu * std::abs(u) * u + Xu * u)) /
             (m - X_u_dot));
      f_psi =
          (((-X_u_dot + Y_v_dot) * u * v + (Nrr * std::abs(r) * r + Nr * r)) /
           (Iz - N_r_dot));
      ...
      ei_u = (integral_step) * (eidot_u + eidot_u_last) / 2 +
             ei_u;  // u integral error
      eidot_u_last = eidot_u;
      ei_psi = (integral_step) * (eidot_psi + eidot_psi_last) / 2 +
               ei_psi;  // r integral error
      ...
      if (starting == 1.0) {
        ...
        // alpha_psi =
        // (pow(std::abs(e_psi0),1-q_psi/p_psi))/(tc_psi*(1-q_psi/p_psi));
        ei_u = -e_u0 / alpha_u;
      ...
      Ka_u = (integral_step) * (Ka_dot_u + Ka_dot_last_u) / 2 +
             Ka_u;  // integral to get the u adaptative gain
      ...
      Ka_psi = (integral_step) * (Ka_dot_psi + Ka_dot_last_psi) / 2 +
               Ka_psi;  // integral to get the r adaptative gain
      ...
      ua_psi = ((-Ka_psi) * pow(std::abs(s_psi), 0.5) * sign_spsi) -
               (k2_psi * s_psi);

      Tx = (udot_d + (alpha_u * eidot_u) - f_u - ua_u) / g_u;  // surge force
      Tz = (rdot_d + (alpha_psi * eidot_psi) + (beta_psi * edot_psi) - f_psi -
            ua_psi) /
           g_psi;  // yaw rate moment
